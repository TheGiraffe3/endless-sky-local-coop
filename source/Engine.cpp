...
Engine::Engine(PlayerInfo &player)
	...
	ammoDisplay{AmmoDisplay(player), AmmoDisplay(player)}, shipCollisions(256u, 32u, CollisionType::SHIP)
{
...
void Engine::Step(bool isActive)
{
	...
	else if(flagship)
	{
		if(isActive)
		{
			const auto [newCenter, newCenterVelocity] = NewCenter(center[0], centerVelocity[0],
				flagship->Center(), flagship->Velocity(), hyperspacePercentage,
				flagship->IsHyperspacing());

			center[0] = newCenter;
			centerVelocity[0] = newCenterVelocity;
		}
	...
	if(Preferences::Has("Cloaked ship outlines"))
		for(const auto &ship : player.Ships())
		{
			...
			outlines[0].emplace_back(ship->GetSprite(), (ship->Position() - center[0]) * zoom, ship->Unit() * zoom,
	...
	{
		outlines[0].emplace_back(flagship->GetSprite(),
			(flagship->Center() - center[0]) * zoom,
	...
	for(int i = 0; i <= splitScreen; i++)
	{
			...
			&& (ship->CargoScanFraction() || ship->OutfitScanFraction()))
		{
			...
			statuses[i].emplace_back(pos, ship->OutfitScanFraction(), ship->CargoScanFraction(),
				0., 10. + max(20., width * .5), Status::Type::SCAN, Angle(pos).Degrees() + 180.);
		}

		if(!Preferences::Has("Ship outlines in HUD"))
			info[i].SetCondition("fast hud sprites");
		if(target[i] && target[i]->IsTargetable() && target[i]->GetSystem() == currentSystem
			&& (ship->CargoScanFraction() || ship->OutfitScanFraction()))
		{
			double width = max(target[i]->Width(), target[i]->Height());
...
void Engine::Draw() const
{
	...
	for(int i = 0; i <= splitScreen; i++)
	{
		...
		Point motionBlur = Preferences::Has("Render motion blur") ? centerVelocity[i] : Point();
		...
		for(const auto &it : statuses[i])
		{
			...
			if(it.outer > 0.)
				RingShader::Draw(pos, radius + 3., 1.5f, it.outer, color[colorIndex], 0.f, it.angle);
			double dashes = (it.type >= Status::Type::SCAN) ? 0. : 20. * min<double>(1., zoom);
			if(it.inner > 0.)
				RingShader::Draw(pos, radius, 1.5f, it.inner, color[5 + colorIndex], dashes, it.angle);
			if(it.disabled > 0.)
				RingShader::Draw(pos, radius, 1.5f, it.disabled, color[10 + colorIndex], dashes, it.angle);
		}
		...
		// Draw the flagship highlight, if any. TODO!!
		//if(highlightSprite)
		//{
...
void Engine::CalculateStep()
{
	...
	if(flagship)
	{
		...
		const auto [newCameraCenter, newCameraVelocity] = NewCenter(center[0], centerVelocity[0],
			flagship->Center(), flagship->Velocity(), hyperspacePercentage,
			isHyperspacing);
		newCenter[0] = newCameraCenter;
		newCenterVelocity[0] = newCameraVelocity;
	}
	if(secondShip.get())
	{
...
void Engine::DrawShipSprites(const Ship &ship)
{
	...
	auto drawObject = [&itemsToDraw, cloak, drawCloaked, fancyCloak](const Body &body) -> void
	{
		...
		if(drawCloaked)
		{
			itemsToDraw[0].AddSwizzled(body, fancyCloak ? 9 : 27, fancyCloak ? 0.5 : 0.25);
			itemsToDraw[1].AddSwizzled(body, fancyCloak ? 9 : 27, fancyCloak ? 0.5 : 0.25);
		}
